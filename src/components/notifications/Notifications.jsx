...
const CheckAllIcon= () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/><polyline points="20 6 13 17 8 12"/></svg>)
const TrashIcon   = () => (<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>)
...
const NotifCard = ({ notif, isRead, onRead, onDismiss }) => {
  const c = typeConfig[notif.type] || typeConfig.upcoming
  return (
    <motion.div
      layout
    ...
    >
      {/* Unread dot */}
      {!isRead && (
        <span className={`absolute top-4 right-4 w-2 h-2 rounded-full ${c.dot}`} />
      )}

      {/* Icon */}
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
        text-lg bg-white/[0.06] border border-white/[0.08]`}>
        {c.icon}
      </div>
      ...
      <div className="flex-1 min-w-0 pr-4">
        ...
        <p className={`text-sm font-semibold truncate ${isRead ? 'text-slate-500' : 'text-slate-100'}`}>
          {notif.title}
        </p>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
const Notifications = () => {
  const { tasks, notifications: prefs } = useTasks()
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 font-[Outfit] p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-100 tracking-tight"
              style={{ fontFamily: 'Syne, sans-serif' }}>Notifications</h1>
